...
| ind_codes | string | 否 | 職缺產業代碼（可用逗號分隔多個，如："123456,654321"） |
| corp_ind_codes | string | 否 | 公司產業代碼（可用逗號分隔多個，如："123456,654321"） |
| type | integer | 否 | 職缺類型 |
| rating | integer | 否 | 最低評分（預設 0） |
| tag | string | 否 | 標籤過濾 |
| overdue | integer | 否 | 是否包含過期職缺（0=不包含，1=包含，預設 0） |
| updated_after | string | 否 | 更新時間起始（含，格式 Y-m-d 或 Y-m-d H:i:s） |
| updated_before | string | 否 | 更新時間結束（含，格式 Y-m-d 或 Y-m-d H:i:s） |
...
- `ind_codes` 搜尋職缺本身的產業；`corp_ind_codes` 搜尋刊登公司的產業，兩者可同時使用（AND 關係）
- `type` 對應職缺的 `type` 欄位（整數完全比對）
- `updated_after` / `updated_before` 依職缺的 `updated_at` 篩選
...
| detail.tools | array | 是 | 工具要求 |
| detail.tools.* | string | 是 | 工具代碼（6位數字）或自訂名稱 |
